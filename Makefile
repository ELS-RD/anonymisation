...
# train a model from manual annotations
	( \
	$(SOURCE_VIRT_ENV); \
	python fine_tune_pre_trained_model.py -i ../case_annotation/data/tc/spacy_manual_annotations -s 0.2 -e 3; \
	)
...
# train a model from manual annotations
	( \
	$(SOURCE_VIRT_ENV); \
	python fine_tune_pre_trained_model.py -i ../case_annotation/data/appeal_court/spacy_manual_annotations  -m ./resources/model -s 0.2 -e 3; \
	)
