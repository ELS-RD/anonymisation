...
def test_get_address():
    ...
    assert get_addresses(text1) == [Offset(6, 53, "ADDRESS_1")]
    text2 = "avant 13 rue Ernest Renan après"
    assert get_addresses(text2) == [Offset(6, len(text2) - 5, "ADDRESS_1")]
    text3 = "avant 20 Avenue André Prothin après"
    assert get_addresses(text3) == [Offset(6, len(text3) - 5, "ADDRESS_1")]
    text4 = "avant 114 avenue Emile Zola après"
    assert get_addresses(text4) == [Offset(6, len(text4) - 5, "ADDRESS_1")]
    text5 = "avant 5 rue Jean Moulin après"
    assert get_addresses(text5) == [Offset(6, len(text5) - 5, "ADDRESS_1")]
    text6 = "avant 85, rue Gabriel Péri après"
    assert get_addresses(text6) == [Offset(6, len(text6) - 5, "ADDRESS_1")]
    text8 = "avant 3, rue Christophe Colomb après"
    assert get_addresses(text8) == [Offset(6, len(text8) - 5, "ADDRESS_1")]
    text9 = "avant 161, rue Andr Bisiaux - ZAC Solvay - Plateau de Haye après"
    assert get_addresses(text9) == [Offset(6, len(text9) - 5, "ADDRESS_1")]
    text10 = "avant 38, boulevard Georges Clémenceau après"
    assert get_addresses(text10) == [Offset(6, len(text10) - 5, "ADDRESS_1")]
    text11 = "avant 35, rue Maurice Flandin après"
    assert get_addresses(text11) == [Offset(6, len(text11) - 5, "ADDRESS_1")]
    text12 = "avant 22 rue Henri Rochefort - 75017 PARIS après"
    assert get_addresses(text12) == [Offset(6, 42, "ADDRESS_1")]
    text13 = "avant 14 Boulevard Marie et Alexandre Oyon - 72100 LE MANS après"
    assert get_addresses(text13) == [Offset(6, 58, "ADDRESS_1")]
    text14 = "avant allée Toto après"
    assert get_addresses(text14) == [Offset(6, len(text14) - 5, "ADDRESS_1")]
    text15 = "avant 14 Boulevard Marie et Alexandre Oyon après"
    assert get_addresses(text15) == [Offset(6, len(text15) - 5, "ADDRESS_1")]
    text16 = "avant 45, rue de Gironis après"
    assert get_addresses(text16) == [Offset(6, 25, "ADDRESS_1")]
    text17 = "avant 10 Boulevard Pasteur à BRY SUR MARNE après"
    assert get_addresses(text17) == [Offset(6, 43, "ADDRESS_1")]
    text18 = "un logement sis 1, rue d'Ebersheim à Strasbourg, moyennant"
    assert get_addresses(text18) == [Offset(16, 49, "ADDRESS_1")]  # TODO should be 47 instead of 49
    text19 = "je me trouve Place de l'Étoile."
    assert get_addresses(text19) == [Offset(13, 31, "ADDRESS_1")]
...
def test_find_address_in_paragraph_block():
    ...
    expected_result = [[], [Offset(0, 30, "ADDRESS")], [Offset(0, 28, "ADDRESS")], [], [], [], [], []]
    offsets3 = [[], [], [Offset(0, 28, "ADDRESS")], [], [], [], [], []]
    offsets4 = [[], [Offset(0, 30, "ADDRESS")], [], [], [], [], [], []]
...
def test_clean_address_offset():
    ...
    assert clean_address_offset(text=text1, offsets=[Offset(0, len(text1) - 1, "ADDRESS")]) == [Offset(21, 26, 'ADDRESS')]
    text2 = "Demeurant 13 rue"
    assert clean_address_offset(text=text2, offsets=[Offset(0, len(text2) - 1, "ADDRESS")]) == [Offset(10, 15, 'ADDRESS')]
    assert clean_address_offsets(texts=[text2], offsets=[[Offset(0, len(text2) - 1, "ADDRESS")]]) == [[Offset(10, 15, 'ADDRESS')]]
